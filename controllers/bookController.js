...
// Display book create fomr on GET.
exports.book_create_get = function(req, res, next){
    async.parallel({
        ...
        if (err) {return next(err);}
        if (results.author_list==null){
            err = new Error('Cannot find a author');
            err.status = 404;
            return next(err);
        }
        // Successful, so render
        res.render('book_form', {title:"Create Book", genre_list: results.genre_list, author_list:results.author_list});
    });
};
    ...
    (req, res, next)=>{
        ...
        if (!errors.isEmpty()){
            ...
            async.parallel({
                ...
                res.render('book_form', {title:'Create Book', author_list:results.authors, genre_list:results.genres, book:book, errors:errors.array()});
            });
            return;
        }
        else{
            // Data from form is valid, so Save book.
            book.save(function(err){
                if (err){return next(err);}
                //sucessfull - redirect to new book record.
    ...
    async.parallel({
        ...
        for (let all_g_iter = 0; all_g_iter < results.genre.length; all_g_iter++){
            for (let book_g_iter=0; book_g_iter<results.book.genre.length; book_g_iter++){
                if (results.book.genre[book_g_iter]._id.toString()===results.genre[all_g_iter]._id.toString()){
                    results.genre[all_g_iter].checked = true;
                }
            }
        }
        res.render('book_form', {title: 'Update books', author_list:results.authors, book:results.book, genre_list:results.genre});
    });
    ...
    (req, res, next) =>{
        ...
        if (!errors.isEmpty()){
            ...
            async.parallel({
                ...
                res.render('book_form', {title:'Update Book', author_list:results.authors, genre_list:results.genres, book:book, errors:errors.array()});
            });
            return;
        }
        else{
